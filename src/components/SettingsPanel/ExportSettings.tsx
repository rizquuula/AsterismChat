...
export function ExportSettings({ state, className = '' }: ExportSettingsProps) {
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <h3 className="text-[13px] font-semibold text-[var(--text-secondary)] uppercase tracking-wide px-1">
        Export Chat
      </h3>
      ...
      <label className="flex items-center gap-3 px-3 py-2 bg-[var(--bg-secondary)] rounded-xl cursor-pointer">
        <input
          type="checkbox"
          checked={includeMetadata}
          onChange={(e) => setIncludeMetadata(e.target.checked)}
          className="w-4 h-4 rounded border-[var(--border)] text-[var(--accent-primary)] focus:ring-[var(--accent-primary)] bg-[var(--bg-primary)]"
        />
        <span className="text-[14px] text-[var(--text-secondary)]">Include metadata</span>
      </label>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[var(--accent-primary)] hover:opacity-90 rounded-xl text-white font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
